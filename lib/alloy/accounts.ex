defmodule Alloy.Accounts do
  ...
  alias Alloy.Accounts.{User, UserToken, UserNotifier}
  ...
  See `Alloy.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Alloy.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  alias Alloy.Accounts.{Account, AccountUser}
  alias Alloy.Accounts.Scope
  ...
  def subscribe_accounts(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Alloy.PubSub, "user:#{key}:accounts")
  end

  defp broadcast_account(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Alloy.PubSub, "user:#{key}:accounts", message)
  end
  ...
  def get_account!(%Scope{} = scope, id) do
    Repo.one!(
      from a in Account,
        join: au in AccountUser,
        on: au.account_id == a.id,
        where: au.user_id == ^scope.user.id and a.id == ^id
    )
  end
  ...
  def create_account(%Scope{} = scope, attrs) do
    Repo.transact(fn ->
      with {:ok, account} <- %Account{} |> Account.changeset(attrs) |> Repo.insert(),
           {:ok, _link} <-
             %AccountUser{}
             |> AccountUser.changeset(%{account_id: account.id, user_id: scope.user.id})
             |> Repo.insert() do
  ...
  def update_account(%Scope{} = scope, %Account{} = account, attrs) do
    # Ensures the scoped user is actually a member of this account
